...
export async function init() {
    try {
        ...
        if (fs.existsSync(storybookMainPath)) {
            throw new Error('❌ .storybook/main.js already exists. Aborting to avoid overwriting.');
        }

        if (fs.existsSync(storybookPreviewPath)) {
            throw new Error('❌ .storybook/preview.js already exists. Aborting to avoid overwriting.');
        }
        ...
        console.log('5. Copy template {main|preview}.js to .storybook/{main|preview}.js');
